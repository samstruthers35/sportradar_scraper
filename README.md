...
# H1 Merging with nflstatR

Strong emphasis **In order to merge this SportRadar data with R there a few steps you have to follow.**
...
Strong emphasis **For example: all_nfl_pbp <-all_nfl_pbp %>% mutate(for_r = paste0(home_team,away_team,week))**
...
Strong emphasis **sports_radar_pbp <- inner_join(all_nfl_pbp, sport_radar, by = c('game_link' = 'for_r', 'week', 'play_id' = 'reference'))**
...
Strong emphasis **pass_checks <- sports_radar_pbp %>% filter(pass == 1, stat_type == "pass", sack.x == 0, qb_scramble == 0) %>% group_by(passer) %>% summarize(plays = n(), yards_gained = sum(yards_gained,na.rm = TRUE)) %>% filter(plays>50) %>% arrange(desc(yards_gained))**

Here is our SportsRadar Results
![Filters For Data](/recents/image_check.png)
